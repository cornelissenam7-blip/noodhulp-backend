...
export async function generateAgent(body,{env=process.env,fetchImpl=fetch}={}){
 ...
 for(let attempt=0;attempt<2;attempt++){
  ...
  if(!feedback.length&&['site','ads'].includes(input.task))feedback=await checkFacts(input,p,{env,fetchImpl,signal});
  if(!feedback.length)break;
  input.corrections=feedback;
  if(attempt===1)throw fail(502,'Het voorstel bevat onbevestigde gegevens en is daarom niet getoond. Je invoer blijft staan. Probeer opnieuw.');
 }
